...
(function () {
  ...
  var REASON_OPTIONS = ["증상 호전", "부작용 의심", "증상 악화로 재처방", "보호자 판단", "기타"];
  ...
  function loadData() {
    try {
      ...
      if (!parsed.child) parsed.child = { name: "", birthDate: "", note: "" };
  ...
  function generateHomeBriefing() {
    ...
    if (recentStopped && daysBetween(recentStopped.discontinuation.date, todayStr()) <= 14) {
      ...
      sub += " " + stoppedMed + josa(stoppedMed, "은", "는") + " 처방 " + recentStopped.prescription.prescribedDays + "일 중 " +
        recentStopped.actualDays + "일만 복용하고 중단했어요.";
    }
  ...
  function generateSummaryBriefing() {
    ...
    if (summaries.length > 0) {
      ...
      if (shortened.length) bits.push('<span class="hl-warn">' + shortened.length + "건은 기간보다 일찍 중단</span>");
  ...
  function renderFooterNote() {
    return '<p class="app-footer-note">아이약쏙은 보호자가 처방 및 실제 복용 이력을 기록하기 위한 도구입니다.<br>약의 복용·중단·변경은 반드시 의사 또는 약사의 지시에 따라 결정해 주세요.</p>';
  }
  ...
  function renderAdd() {
    ...
    var typeGrid = ["symptom", "hospital", "prescription", "intake", "discontinuation"].map(function (t) {
  ...
  function bindAdd() {
    ...
    root.querySelectorAll('input[name="symptomCustom"], input[name="reasonCustom"]').forEach(function (el) {
      el.addEventListener("input", function () {
        var group = el.name === "symptomCustom" ? "symptom" : "reason";
        var hidden = root.querySelector('input[type="hidden"][name="' + group + '"]');
  ...
  function compareBarHtml(prescribedDays, actualDays, ongoing) {
    ...
    if (ongoing) {
      ...
      deltaHtml = '<span class="delta delta--short">' + Math.abs(diff) + '일 단축</span>';
    } else {
  ...
  function renderSummary() {
    ...
    if (antibiotics.length > 0) {
      html += '<div class="antibiotic-panel">' +
        ...
        '</div>' +
        '<div class="antibiotic-panel__desc">항생제는 임의로 중단하거나 기간을 늘리지 않는 것이 중요해요.</div>' +
  ...
  function renderSettings() {
    ...
    html += '<div class="settings-group">' +
      '<div class="notice-box">' +
        ...
        '<span>아이약쏙은 보호자가 처방 및 실제 복용 이력을 기록하기 위한 도구입니다. 약의 복용, 중단, 변경 여부는 반드시 의사 또는 약사의 지시에 따라 결정해 주세요.</span>' +
      '</div>' +
    '</div>';
  ...
  function generateSampleData() {
    ...
    if (!state.data.child.name) {
      state.data.child = { name: "서준", birthDate: addDaysStr(today, -2 * 365), note: "특이 알레르기 없음" };
    }

    var sample = [
      { type: "symptom", date: d(-10), symptom: "기침", severity: "보통", memo: "밤에 기침이 심해짐" },
      { type: "hospital", date: d(-10), hospitalName: "튼튼소아과", diagnosis: "급성 기관지염", doctorMemo: "호전 없으면 3일 후 재방문" },
      { type: "prescription", date: d(-10), medName: "코푸시럽", category: "기침약", prescribedDays: 5, timesPerDay: 3, doctorInstruction: "식후 30분 복용" },
      { type: "intake", date: d(-10), medName: "코푸시럽", startDate: d(-10), endDate: d(-7), status: "완료" },

      { type: "symptom", date: d(-6), symptom: "콧물", severity: "보통", memo: "누런 콧물로 변함" },
      { type: "hospital", date: d(-6), hospitalName: "튼튼소아과", diagnosis: "급성 비염", doctorMemo: "항생제로 변경, 경과 관찰" },
      { type: "prescription", date: d(-6), medName: "세프클러현탁액", category: "항생제", prescribedDays: 5, timesPerDay: 3, doctorInstruction: "5일 모두 복용 권장" },
      { type: "intake", date: d(-6), medName: "세프클러현탁액", startDate: d(-6), endDate: d(-4), status: "중단" },
      { type: "discontinuation", date: d(-4), medName: "세프클러현탁액", reason: "증상 호전", reaction: "특별한 반응 없음" },

      { type: "symptom", date: d(-2), symptom: "구토", severity: "심함", memo: "저녁식사 후 1회 구토" },
      { type: "hospital", date: d(-1), hospitalName: "튼튼소아과", diagnosis: "급성 위장관염", doctorMemo: "수분 섭취 권장, 3일치 정장제 처방" },
      { type: "prescription", date: d(-1), medName: "정장제", category: "소화기약", prescribedDays: 3, timesPerDay: 2, doctorInstruction: "설사 지속 시 재방문" },
      { type: "intake", date: d(-1), medName: "정장제", startDate: d(-1), endDate: "", status: "복용중" }
    ];
