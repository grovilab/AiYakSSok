...
(function () {
  ...
  function medStatusCardHtml(i, summaries) {
    ...
    var doseHtml = matched && matched.prescription.dose ? '<span class="med-status-card__dose">' + escapeHtml(matched.prescription.dose) + '</span>' : "";
    var antiHtml = matched && isAntibiotic(matched.prescription) ? '<span class="badge badge-antibiotic">항생제</span>' : "";
    var bodyHtml;
    if (matched) {
      var pct = Math.min((days / matched.prescription.prescribedDays) * 100, 100);
      bodyHtml =
        '<span class="med-status-card__days"><strong>복용 ' + days + '일째</strong><span class="med-status-card__of">처방 ' + matched.prescription.prescribedDays + '일 중</span></span>' +
        '<span class="med-status-card__bar"><span class="med-status-card__bar-fill" style="width:' + pct + '%"></span></span>' +
        '<span class="med-status-card__meta">' + formatDateShort(i.startDate) + ' 시작 · 하루 ' + matched.prescription.timesPerDay + '회</span>';
    } else {
      bodyHtml =
        '<span class="med-status-card__days"><strong>복용 ' + days + '일째</strong></span>' +
        '<span class="med-status-card__meta">' + formatDateShort(i.startDate) + '부터 기록 중</span>';
    }
    return '<button type="button" class="med-status-card" data-action="open-detail" data-id="' + i.id + '">' +
      '<span class="med-status-card__head">' +
        '<span class="med-status-card__name">' + escapeHtml(i.medName) + doseHtml + antiHtml + '</span>' +
        '<span class="badge badge-status status-ongoing">복용중</span>' +
      '</span>' +
      bodyHtml +
    '</button>';
  }
  ...
  function renderFooterNote() {
    return '<div class="app-footer">' +
      '<p class="app-footer-tagline">아이약쏙은 아이의 진료 기억과 복약 흐름을 보호자가 차분히 정리하는 기록 도구예요.</p>' +
      '<p class="app-footer-note">' + escapeHtml(SAFETY_NOTICE_FULL) + '</p>' +
    '</div>';
  }
  ...
  /* 에피소드 단계 표시 (표시 전용, buildEpisodes 결과만 읽음):
     증상 → 병원 방문 → 처방 → 실제 복용 → 완료/중단 → 반응 */
  function episodeStepsHtml(ep) {
    function has(type) { return ep.items.some(function (r) { return r.type === type; }); }
    var intakesInEp = ep.items.filter(function (r) { return r.type === "intake"; });
    var hasStopped = intakesInEp.some(function (r) { return r.status === "중단"; }) || has("discontinuation");
    var hasDone = intakesInEp.some(function (r) { return r.status === "완료"; });
    var endOn = hasStopped || hasDone;
    var endLabel = hasStopped ? "중단" : (hasDone ? "완료" : "완료·중단");

    var steps = [
      { label: "증상", on: has("symptom"), color: "var(--symptom)" },
      { label: "병원", on: has("hospital"), color: "var(--hospital)" },
      { label: "처방", on: has("prescription"), color: "var(--prescription)" },
      { label: "복용", on: intakesInEp.length > 0, color: "var(--intake)" },
      { label: endLabel, on: endOn, color: hasStopped ? "var(--discontinue)" : "var(--success)" },
      { label: "반응", on: has("reaction"), color: "var(--reaction)" }
    ];

    return '<div class="episode-steps" aria-hidden="true">' + steps.map(function (s, idx) {
      return (idx > 0 ? '<span class="episode-steps__link' + (s.on ? " is-on" : "") + '"></span>' : "") +
        '<span class="episode-steps__step' + (s.on ? " is-on" : "") + '" style="--step:' + s.color + '">' +
          '<span class="episode-steps__dot"></span>' +
          '<span class="episode-steps__label">' + s.label + '</span>' +
        '</span>';
    }).join("") + '</div>';
  }
  ...
  function renderSummary() {
    ...
    // 1) 자동 브리핑 — 기록을 바탕으로 자동 정리한 문단 (+ 핵심 지표 축소 통합)
    var miniStatsHtml =
      '<div class="summary-briefing__stats">' +
        miniStat(hospitals.length, "병원 방문") +
        miniStat(symptoms.length, "증상") +
        miniStat(prescriptions.length, "처방") +
        miniStat(intakes.length, "실제 복용") +
        miniStat(antibiotics.length, "항생제") +
        miniStat(getRecords("discontinuation").length, "중단") +
      '</div>';
    ...
    if (briefingText) {
      ...
      // 브리핑 문단이 없어도 지표는 유지 (기존 stat-grid 대체)
      html += '<div class="summary-briefing">' + miniStatsHtml + '</div>';
    }

    // 2) 현재 복용 중인 약 — 진료 시 가장 먼저 보여줄 정보
    var ongoingIntakes = sortByDateDesc(intakes.filter(function (i) { return i.status === "복용중"; }));
    html += '<div class="screen-section">' +
      '<div class="screen-section__head"><span class="screen-section__title">현재 복용 중인 약</span></div>';
    if (ongoingIntakes.length === 0) {
      html += '<div class="card u-muted" style="font-size:12.5px;">현재 복용 중으로 등록된 약이 없어요.</div>';
  ...
  /* 자동 브리핑 카드 하단에 통합되는 축소형 지표 (표시 전용) */
  function miniStat(num, label) {
    return '<span class="mini-stat"><span class="mini-stat__num">' + num + '</span><span class="mini-stat__label">' + label + '</span></span>';
  }
